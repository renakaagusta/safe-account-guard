...
contract SafeGuard is BaseGuard {
    ...
    uint256 public costPerTransaction;
    ...
    event FunctionRejected(
        address indexed target,
        bytes4 indexed functionSelector
    );
    event FunctionProposed(
        address indexed target,
        bytes4 indexed functionSelector
    );

    error FunctionNotAllowed(address target, bytes4 functionSelector);
    error InsufficientBalance(address sender);

    constructor() {
        costPerTransaction = 5 * 10 ** 4;
    }
    ...
    function updateCurrencyToken(address _currencyToken) external {
        currencyToken = _currencyToken;
    }

    function updateCostPerTransaction(uint256 _costPerTransaction) external {
        costPerTransaction = _costPerTransaction;
    }
    ...
    function checkTransaction(
        ...
        if ((currencyToken != address(0) && serviceManager != address(0)) && (to != currencyToken || to != serviceManager)) {
            if (rejectedFunctions[to][functionSelector] == false) {
                revert FunctionNotAllowed(to, functionSelector);
            }

            if (balances[msg.sender] < costPerTransaction) {
                revert InsufficientBalance(msg.sender);
            }

            balances[msg.sender] += costPerTransaction;
